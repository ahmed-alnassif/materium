...
sealed class _InteractiveListItemStates extends _ListItemStates
  ...
  factory _InteractiveListItemStates.fromWidgetStates(
    ...
    return states.contains(.disabled)
        ? .disabled(isFirst: isFirst, isLast: isLast)
        : .enabled(
            isFirst: isFirst,
            isLast: isLast,
            isHovered: states.contains(.hovered),
            isFocused: states.contains(.focused),
            isPressed: states.contains(.pressed),
          );
  }
...
class _ListItemInteractionState extends State<ListItemInteraction> {
  ...
  Widget build(BuildContext context) {
    ...
    final overlayColor = MixedWidgetStateLayerColor<ListItemStates>.from(
      (widgetStates) => _InteractiveListItemStates.fromWidgetStates(
        widgetStates,
